    ...
    for i, data in enumerate(train_loader):
        ...
        if(counter > discrim_iters):
            ...
            for j in range(gen_iters):
                ...
                reg_uv = 0 * torch.linalg.norm(noise_uv_z, axis=1).mean()
                reg_bg = 0 * torch.linalg.norm(noise_bg_z, axis=1).mean()

                (real_loss + reg_uv + albedo_loss + reg_bg).backward()
                optimizer_G.step()

                if(j == (gen_iters - 1) or j == 0):
                    new_out = "epoch "+ str(epoch) + " cur_nimg " + str(i) +" generator: fake "+ str(real_loss.item()) + " albedo " + str(albedo_loss.item()) + " reg uv " + str(reg_uv.item()) + "reg bg " + str(reg_bg.item())
                    logging.info(new_out)
