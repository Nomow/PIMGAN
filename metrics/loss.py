...
class StyleGAN2Loss(Loss):
    def __init__(self, device, G_mapping, G_synthesis, D, augment_pipe=None, style_mixing_prob=0.0, r1_gamma=10, pl_batch_shrink=2, pl_decay=0.01, pl_weight=2):
        super().__init__()
        self.device = device
    ...
    def run_G(self, z, c, sync, style_mixing=0.9):
        with misc.ddp_sync(self.G_mapping, sync):
            ws = self.G_mapping(z, c)
            if style_mixing > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
                    cutoff = torch.where(torch.rand([], device=ws.device) < style_mixing, cutoff, torch.full_like(cutoff, ws.shape[1]))
...
class PIMGANLoss(Loss):
    ...
    def run_G(self, z, c, sync, style_mixing=0.9):
        with misc.ddp_sync(self.G_mapping, sync=True):
            ws = self.G_mapping(z, c)
            if style_mixing > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    cutoff = torch.empty([], dtype=torch.int64, device=ws.device).random_(1, ws.shape[1])
    ...
    def run_D(self, img, c, sync=True):
        if self.augment_pipe is not None:
            img = self.augment_pipe(img)
        with misc.ddp_sync(self.D, sync):
            logits = self.D(img, c)
...
class FaceIDLoss(nn.Module):
    ...
    def resize_n_crop1(self, image, M, resize_size = 256):
        # image: (b, c, h, w)
        # M   :  (b, 2, 3)
        return warp_affine(
            image, M, dsize=(resize_size, resize_size)
        )
